...
def custom_locale_dir():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 创建自定义翻译文件
        custom_zh = {"test": {"message": "测试消息: {name}", "greeting": "你好"}}

        custom_en = {"test": {"message": "Test message: {name}", "greeting": "Hello"}}
...
def test_text_formatting():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        text = manager.get_text(
            "core.errors.module_not_found", module_name="TestModule"
        )
